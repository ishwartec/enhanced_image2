    ...
    function widgetDef(editor) {
        ...
        return {
            ...
            data: function() {
                ...
                // Code started by ISHWAR  
                // Setting data for margin 

//                this.data.marginright = this.element.getStyle('margin-right').trim().replace("px", "");
//                this.data.marginleft = this.element.getStyle('margin-left').trim().replace("px", "");
//                this.data.margintop = this.element.getStyle('margin-top').trim().replace("px", "");
//                this.data.marginbottom = this.element.getStyle('margin-bottom').trim().replace("px", "");

                // End setting data for margin 
                // Code End by ISHWAR
        ...
        stateShifter: function(editor) {
            ...
            var stateActions = {
                ...
                hasCaption: function(shift, oldValue, newValue) {
                    ...
                    // Setting Padding
                    shift.element.setStyle('padding-right', shift.newData.paddingright + 'px');
                    shift.element.setStyle('padding-left', shift.newData.paddingleft + 'px');
                    shift.element.setStyle('padding-top', shift.newData.paddingtop + 'px');
                    shift.element.setStyle('padding-bottom', shift.newData.paddingbottom + 'px');

                    // Setting margin
                    shift.element.setStyle('margin-right', shift.newData.marginright + 'px');
                    shift.element.setStyle('margin-left', shift.newData.marginleft + 'px');
                    shift.element.setStyle('margin-top', shift.newData.margintop + 'px');
                    shift.element.setStyle('margin-bottom', shift.newData.marginbottom + 'px');

                    // Condition added .....   If image have not any caption 
                    // Just removing the def style if and only if figure DOM object availability

                    if (shift.changed.hasCaption == true) {
                        // Removing style from IMG if and only if image caption have been chosen
                        shift.element.removeAttribute("style")
                        if (i(shift.element.$.attributes.class, "value")) {
                            var styleClass = shift.element.$.attributes.class.value;
                            ...
                            for (var chunkofStyleClassArray in styleClassArray) {
                                ...
                                shift.element.removeClass(selectedStyle);
                            }
                        }
                    }
                    ...
                    if (i(shift.element.$.attributes.class, "value")) {
                        ...
                        for (var chunkofStyleClassArray in styleClassArray) {
                            ...
                            if (selectedStyle == 'image') {
                                ...
                                if (shift.newData.customclassaddonfield)
                                    shift.element.removeClass(selectedStyle);
                            }
                        }
                    ...
                    // If class addon value is not empty or undefined or null 
                    if (shift.newData.customclassaddonfield && shift.changed.hasCaption != true)
                        shift.element.addClass(shift.newData.customclassaddonfield);
